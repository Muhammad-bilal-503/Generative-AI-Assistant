...
APP_MAPPINGS = {
    ...
    "paint": "mspaint",
    # YAHAN USERNAME THEEK KAR DIYA GAYA HAI
    "vs code": "C:\\Users\\Muhammad Bilal\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe",
    "postman": "C:\\Users\\Muhammad Bilal\\AppData\\Local\\Postman\\Postman.exe"
}
...
# App control
# Naya, Behtar aur Reliable open_app Function

@function_tool
async def open_app(app_title: str) -> str:
    """
    Opens a specified application on the Windows PC using its shortcut name or command.
    Use this tool when the user says "open Chrome", "launch Notepad", or similar commands.
    """
    app_title = app_title.lower().strip()
    app_command = APP_MAPPINGS.get(app_title, app_title)
    
    try:
        logger.info(f"Attempting to open '{app_title}' with command: '{app_command}'")
        # os.startfile Windows par applications kholne ka sab se reliable tareeqa hai
        os.startfile(app_command)
        
        # Thora waqfa dein taake window khul sake
        await asyncio.sleep(2) 
        
        # Check karein ke window khuli hai ya nahi
        if await focus_window(app_title):
            return f"Application '{app_title}' has been successfully launched."
        else:
            # Agar focus na ho, to yeh aam tor par pehle se khuli hone ki wajah se hota hai
            return f"Application '{app_title}' is now open."

    except FileNotFoundError:
        logger.error(f"Command '{app_command}' not found for '{app_title}'. Please check the path in APP_MAPPINGS.")
        return f"Sorry, I could not find the application '{app_title}'. Please ensure it is installed correctly."
    except Exception as e:
        logger.error(f"An unexpected error occurred while opening '{app_title}': {e}")
        return f"I encountered an error while trying to open '{app_title}'."
