...
import asyncio
from Jarvis_window_CTRL import open_app  # Yeh line 'open' tool ko bulayegi
...
class SafeController:
    def __init__(self):
        ...
        self.log_file = "controller_log.txt"  # <-- YEH LINE ADD KAR DEIN
...
# Yeh naya, behtar function istemal karein

@function_tool
async def search_on_chrome(query: str):
    """
    Opens Google Chrome, types a search query, and presses Enter.
    Use this tool when the user wants to search for something on Chrome.

    Args:
        query (str): The text to search for on Google Chrome.
    """
    try:
        # Kaam 1: Chrome ko kholna
        print(f"Opening Chrome to search for: {query}")
        await open("Chrome")
        
        # Kaam 2: 3 second intezar karna
        await asyncio.sleep(3)

        # Kaam 3: Text type karna
        await type_text_tool(query)
        ...
        return f"An error occurred while trying to search on Chrome: {e}"
